...
class FilterModal {
  ...
  static Future<void> showAddModal(BuildContext context, List<Spent> filterSpentList, Function(List<Spent>) updateSpentList) async {
        ...
        return AlertDialog(
              ...
              children: [
                TextFormField(
                  ...
                  onTap: () async {
                    ...
                    if (pickedDate != null) {
                      startDateController.text = "${pickedDate.toLocal()}".split(' ')[0]; // Format date
                    }
                  },
                ),
                ...
                TextFormField(
                  ...
                  onTap: () async {
                    ...
                    if (pickedDate != null) {
                      endDateController.text = "${pickedDate.toLocal()}".split(' ')[0]; // Format date
                    }
                  },
                ),
              ],
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () {
                List<Spent> filteredList = _filterSpentList(filterSpentList, startDateController.text, endDateController.text); // Appeler la fonction de filtrage
  ...
  static List<Spent> _filterSpentList(List<Spent> spentList, String startDate, String endDate) {
    DateTime? filterStartDate = DateTime.tryParse(startDate); // Convertir la date en DateTime
    DateTime? filterEndDate = DateTime.tryParse(endDate); // Convertir la date en DateTime
    if (filterStartDate != null && filterEndDate != null) {
      List<Spent> filteredList = spentList.where((spent) {
      DateTime spentDate = DateTime.parse(spent.createdAt).toLocal(); // Convertir en DateTime local
      
      // Vérifier si la date est dans l'intervalle
      return spentDate.isAfter(filterStartDate.subtract(const Duration(days: 1))) && 
